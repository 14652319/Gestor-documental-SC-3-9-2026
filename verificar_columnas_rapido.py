"""
Script rápido para verificar solo columnas y primeras filas
"""
import pandas as pd
...
print("=" * 80)
print("🔍 VERIFICACIÓN RÁPIDA DE COLUMNAS Y PRIMERAS FILAS")
print("=" * 80)
...
try:
    ...
    print(f"✅ Columnas originales en DIAN:")
...
try:
    ...
    print(f"✅ Columnas originales en ACUSES:")
    ...
    if cufe_col:
        print("\n" + "=" * 80)
        print("🔍 COMPARACIÓN DIRECTA (primeros 3)")
        print("=" * 80)
        ...
        if dian_cufe_col:
            ...
            if coincidencias:
                print(f"\n✅ SÍ HAY COINCIDENCIAS en las primeras 10 filas!")
                for cufe in list(coincidencias)[:3]:
                    print(f"      {cufe[:60]}...")
            else:
                print(f"\n❌ NO HAY COINCIDENCIAS en las primeras 10 filas")
                print("\n🔍 Primer CUFE de cada archivo:")
                print(f"\n   DIAN:")
                print(f"   {list(dian_cufes)[0] if dian_cufes else 'VACÍO'}")
                print(f"\n   ACUSES:")
                print(f"   {list(acuses_cufes)[0] if acuses_cufes else 'VACÍO'}")
