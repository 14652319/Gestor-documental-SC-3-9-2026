...
def main():
    ...
    directorio_backups = Path(r"C:\Users\Usuario\Desktop\Gestor Documental\PAQUETES_TRANSPORTABLES\Backup")
    timestamp_base = datetime.now().strftime('%Y%m%d')
    
    backups_encontrados = []
    if directorio_backups.exists():
        for item in directorio_backups.iterdir():
            if timestamp_base in item.name and item.is_dir():
                backups_encontrados.append(item.name)
    ...
    if not backups_encontrados:
        ...
        if directorio_backups.exists():
            for item in directorio_backups.iterdir():
                if ('BACKUP' in item.name or 'GESTOR' in item.name) and item.is_dir():
                    backups_encontrados.append(item.name)
    ...
    for nombre_backup in [backup_recuperacion, backup_transportable]:
        ...
        # Ruta completa en la carpeta de backups
        ruta_backup = directorio_backups / nombre_backup
        
        # Determinar subdirectorio según tipo
        if 'TRANSPORTABLE' in nombre_backup:
            archivo_salida = str(ruta_backup / 'backup_gestor_documental.backup')
        else:
            archivo_salida = str(ruta_backup / 'base_datos' / 'backup_gestor_documental.backup')
