...
def solicitar_correccion_documento(documento_id):
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"🔄 SOLICITUD DE CORRECCIÓN INICIADA")
        print(f"{'='*80}")
        ...
        if usuario_id:
            try:
                ...
                if result and result[0]:
                    ...
                    print(f"✅ DEBUG: Correo encontrado por ID: {correo_destinatario}")
            except Exception as e:
                print(f"⚠️ DEBUG: Error buscando por ID: {e}")
        ...
        if not correo_destinatario and usuario_nombre:
            try:
                ...
                if result and result[0]:
                    correo_destinatario = result[0]
                    print(f"✅ DEBUG: Correo encontrado por nombre: {correo_destinatario}")
            except Exception as e:
                print(f"⚠️ DEBUG: Error buscando por nombre: {e}")
        ...
        if not correo_destinatario and usuario_nombre:
            try:
                ...
                if user_int and user_int.email:
                    ...
                    print(f"✅ DEBUG: Correo encontrado en usuarios_internos: {correo_destinatario}")
            except Exception as e:
                print(f"⚠️ DEBUG: Error buscando en usuarios_internos: {e}")
        
        # Si no se encuentra correo, retornar error
        if not correo_destinatario:
            print(f"❌ DEBUG: No se encontró correo para usuario={usuario_nombre}, id={usuario_id}")
            return jsonify({
                'success': False,
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>🔄 Código de Corrección de Documento</h1>
        ...
        try:
            msg = Message(
                subject=f"🔄 Código de Corrección - Documento {documento.nombre_archivo}",
            ...
            print("\n" + "="*80)
            print(f"⚠️ CÓDIGO DE CORRECCIÓN GENERADO (Correo falló)")
            print("="*80)
            print(f"📧 Destinatario: {correo_destinatario}")
            print(f"👤 Usuario: {usuario}")
            print(f"📄 Documento: {documento.nombre_archivo}")
            print(f"🔑 TOKEN: {token}")
            print(f"❌ Error: {str(e_mail)}")
            print("="*80 + "\n")
        
        log_security(f"CORRECCIÓN SOLICITADA | doc={documento_id} | usuario={usuario} | token_id={token_correccion.id}")
...
def validar_correccion_documento(token_id):
    ...
    try:
        ...
        try:
            ...
            # Ruta anterior
            ruta_anterior_completa = os.path.join(base_dir, documento.ruta_archivo)
            directorio_anterior = os.path.dirname(ruta_anterior_completa)
            ...
            extension_archivo = os.path.splitext(ruta_anterior_completa)[1]
            nuevo_nombre_archivo = f"{nuevo_nombre_base}{extension_archivo}"
            nueva_ruta_completa = os.path.join(nuevo_directorio, nuevo_nombre_archivo)
            
            if os.path.exists(ruta_anterior_completa):
                shutil.move(ruta_anterior_completa, nueva_ruta_completa)
            
            # Actualizar ruta en BD (relativa)
            ...
            documento.nombre_archivo = nuevo_nombre_archivo
            
            # Mover adjuntos
            adjuntos = AdjuntoDocumento.query.filter_by(documento_id=documento.id).all()
            
            for adjunto in adjuntos:
                ruta_adjunto_anterior = os.path.join(base_dir, adjunto.ruta_archivo)
                
                if os.path.exists(ruta_adjunto_anterior):
                    # Extraer etiqueta del nombre anterior (después de _ANEXO_)
                    nombre_anterior = os.path.basename(ruta_adjunto_anterior)
                    extension_adj = os.path.splitext(nombre_anterior)[1]
                    
                    # Si el nombre tiene formato CO-TIPO-CONSEC_ANEXO_etiqueta, extraer etiqueta
                    if '_ANEXO_' in nombre_anterior:
                        etiqueta = nombre_anterior.split('_ANEXO_', 1)[1].replace(extension_adj, '')
                    else:
                        etiqueta = nombre_anterior.replace(extension_adj, '')
                    
                    # Nuevo nombre: nuevo_prefijo_ANEXO_etiqueta
                    nuevo_nombre_adjunto = f"{nuevo_nombre_base}_ANEXO_{etiqueta}{extension_adj}"
                    nueva_ruta_adjunto = os.path.join(nuevo_directorio, nuevo_nombre_adjunto)
                    
                    shutil.move(ruta_adjunto_anterior, nueva_ruta_adjunto)
                    
                    # Actualizar BD
                    adjunto.nombre_archivo = nuevo_nombre_adjunto
                    adjunto.ruta_archivo = os.path.join(
                        empresa.siglas,
                        año,
                        mes,
                        centro.codigo,
        ...
        log_security(f"CORRECCIÓN APLICADA | doc={documento.id} | usuario={usuario} | token_id={token_id}")
