    ...
    try:
        ...
        query = text("""
            ...
            LEFT JOIN (
                ...
                ORDER BY usuario_id, fecha_ultima_actividad DESC
            ) sa ON u.id = sa.usuario_id
        ...
        for user in usuarios:
            ...
            minutos_inactivo = user.minutos_inactivo
            if user.conectado and minutos_inactivo is not None:
                if minutos_inactivo <= 5:
                    estado_conexion = "🟢 Conectado"
                elif minutos_inactivo <= 10:
            ...
            pais = user.pais or ''
            ubicacion = f"{ciudad}, {pais}".strip(', ') or 'N/A'
            ...
            resultado.append({
                ...
                'minutos_inactivo': int(minutos_inactivo) if minutos_inactivo else 0
            })
...
def ips_tiempo_real():
    ...
    try:
        ...
        for row in ips_data:
            ...
            # Verificar si está bloqueada usando SQL directo (separar fuente)
            en_negras = db.session.execute(
                text("SELECT EXISTS (SELECT 1 FROM ips_negras WHERE ip = :ip)"), {'ip': ip}
            ).scalar()
            en_sospechosas = db.session.execute(
                text("SELECT EXISTS (SELECT 1 FROM ips_sospechosas WHERE ip = :ip)"), {'ip': ip}
            ).scalar()
            bloqueada = bool(en_negras or en_sospechosas)
            ...
            resultado.append({
                ...
                'usuarios': usuarios,
                'bloqueada': bloqueada,
                'en_negras': bool(en_negras),
                'en_sospechosas': bool(en_sospechosas),
...
def listar_archivos_log():
    ...
    try:
        ...
        # Directorios a escanear: logs/ + raíz del proyecto
        dirs_a_escanear = []
        if os.path.exists(logs_dir):
            dirs_a_escanear.append(logs_dir)
        # También buscar .log sueltos en la raíz
        dirs_a_escanear.append(app_root)
        
        if not dirs_a_escanear:
            return jsonify({'success': True, 'data': []})
        
        archivos_log = []
        nombres_vistos = set()  # evitar duplicados
        for directorio in dirs_a_escanear:
            ...
            for archivo in os.listdir(directorio):
                if not archivo.endswith('.log'):
                    continue
                if archivo in nombres_vistos:
                    continue
                # Si está en la raíz, solo incluir .log de primer nivel (no subcarpetas)
        ...
        # Ordenar: primero con contenido, luego por nombre
        archivos_log.sort(key=lambda x: (-x['lineas'], x['nombre']))
...
def leer_log_seguridad():
    ...
    try:
        ...
        # Buscar el archivo: primero en logs/, luego en la raíz
        ruta_archivo = os.path.join(logs_dir, archivo)
        if not os.path.exists(ruta_archivo):
            ruta_raiz = os.path.join(app_root, archivo)
            if os.path.exists(ruta_raiz):
                ruta_archivo = ruta_raiz
...
def gestionar_ip():
    ...
    try:
        ...
        if accion == 'agregar_blanca':
            ...
            # Quitar de ips_negras
            result_neg = db.session.execute(text("DELETE FROM ips_negras WHERE ip = :ip"), {'ip': ip})
            # También quitar de ips_sospechosas si estaba allí
            result_sosp = db.session.execute(text("DELETE FROM ips_sospechosas WHERE ip = :ip"), {'ip': ip})
            db.session.commit()
            
            total_eliminadas = result_neg.rowcount + result_sosp.rowcount
            if total_eliminadas > 0:
                fuentes = []
                if result_neg.rowcount > 0:
                    fuentes.append('lista negra')
                if result_sosp.rowcount > 0:
                    fuentes.append('lista sospechosas')
                resultado = {'success': True, 'message': f'IP {ip} removida de: {" y ".join(fuentes)}'}
            else:
                resultado = {'success': False, 'message': f'IP {ip} no está en lista negra ni sospechosas'}
