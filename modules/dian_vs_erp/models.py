...
class TokenEliminacionDatos(db.Model):
    """
    Tokens de validación para eliminación de datos masivos
    Seguridad mediante código enviado por correo electrónico
    Fecha creación: 16 de Febrero de 2026
    """
    __tablename__ = 'tokens_eliminacion_dian_erp'
    
    id = db.Column(db.Integer, primary_key=True)
    token = db.Column(db.String(6), nullable=False, index=True)  # Código de 6 dígitos
    usuario_solicitante = db.Column(db.String(100), nullable=False, index=True)
    email_destino = db.Column(db.String(255), nullable=False)
    
    # Parámetros de eliminación
    tipo_rango = db.Column(db.String(20), nullable=False)  # 'dias', 'meses', 'año'
    fecha_inicio = db.Column(db.Date)
    fecha_fin = db.Column(db.Date)
    
    # Archivos/tablas a eliminar (JSON string con array)
    archivos_eliminar = db.Column(db.Text, nullable=False)  # ["dian", "erp_fn", "erp_cm", "acuses"]
    
    # Estado del token
    usado = db.Column(db.Boolean, default=False, nullable=False, index=True)
    fecha_creacion = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    fecha_expiracion = db.Column(db.DateTime, nullable=False)  # 10 minutos de validez
    fecha_uso = db.Column(db.DateTime)
    
    # Resultado de la eliminación (si se ejecutó)
    registros_eliminados = db.Column(db.Integer)
    resultado_json = db.Column(db.Text)  # JSON con detalles por tabla
    ip_solicitud = db.Column(db.String(50))
    ip_confirmacion = db.Column(db.String(50))
