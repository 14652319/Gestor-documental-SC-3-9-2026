...
import secrets, os, logging, logging.handlers
...
fh = logging.handlers.RotatingFileHandler(
    "logs/security.log", maxBytes=5*1024*1024, backupCount=5, encoding="utf-8"
)
            ...
            <div class="container">
                ...
                <div class="content">
                    ...
                    <div style="text-align: center; margin: 30px 0;">
                        ...
                        <a href="{os.getenv('APP_BASE_URL', 'http://192.168.11.205:8099')}" class="cta-button">
                            🌐 Acceder al Sistema
                        </a>
                    </div>
                </div>
...
Acceso al sistema: {os.getenv('APP_BASE_URL', 'http://192.168.11.205:8099')}
...
def enviar_alerta_seguridad_telegram(tipo_alerta, detalles):
    ...
    try:
        ...
        # 📋 Si es IP_BLOQUEADA, agregar listado de todas las IPs bloqueadas
        if tipo_alerta == 'IP_BLOQUEADA':
            try:
                # Consultar todas las IPs bloqueadas
                ips_bloqueadas = IPListaNegra.query.all()
                ips_sospechosas_bloqueadas = IPSospechosa.query.filter_by(bloqueada=True).all()
                
                # Combinar ambas listas (evitar duplicados)
                ips_set = set()
                ips_info = []
                
                for ip_neg in ips_bloqueadas:
                    if ip_neg.ip not in ips_set:
                        ips_set.add(ip_neg.ip)
                        motivo = ip_neg.motivo if hasattr(ip_neg, 'motivo') else 'No especificado'
                        ips_info.append((ip_neg.ip, motivo))
                
                for ip_sosp in ips_sospechosas_bloqueadas:
                    if ip_sosp.ip not in ips_set:
                        ips_set.add(ip_sosp.ip)
                        motivo = ip_sosp.motivo_bloqueo if hasattr(ip_sosp, 'motivo_bloqueo') else 'Intentos fallidos'
                        ips_info.append((ip_sosp.ip, motivo))
                
                # Agregar sección de IPs bloqueadas al mensaje
                if ips_info:
                    mensaje += f"\n\n🚫 *TODAS LAS IPS BLOQUEADAS ACTUALMENTE:*\n"
                    mensaje += f"*Total:* {len(ips_info)} IP(s)\n\n"
                    
                    for idx, (ip, motivo) in enumerate(ips_info[:10], 1):  # Máximo 10 para no hacer mensaje muy largo
                        mensaje += f"{idx}. `{ip}` - {motivo[:30]}\n"
                    
                    if len(ips_info) > 10:
                        mensaje += f"\n... y {len(ips_info) - 10} IP(s) más\n"
                ...
                # No fallar el envío si hay error al listar
                pass
...
# ⭐ NUEVO: Tabla de alertas de vencimiento SAGRILAFT (Enero 29, 2026)
class AlertaVencimiento(db.Model):
    ...
    radicado = db.Column(db.String(20), nullable=False)
    fecha_primera_alerta = db.Column(db.DateTime, nullable=True)
...
def api_login():
    ...
    try:
        if tercero_usuario and hasattr(tercero_usuario, 'correo_electronico') and tercero_usuario.correo_electronico:
            session['email'] = tercero_usuario.correo_electronico
        else:
            session['email'] = ''
    except:
        session['email'] = ''
...
def telegram_webhook():
    ...
    - /desbloquear [IP] - Desbloquear una IP bloqueada
    - /ayuda - Mostrar comandos disponibles
    """
    try:
        # Obtener actualización de Telegram
        update = request.get_json()
        log_security(f"TELEGRAM WEBHOOK RECIBIDO | update_id={update.get('update_id', 'N/A')}")
        
        # Importar módulo del bot
        from telegram_bot import procesar_update_telegram
        
        # Procesar update (maneja comando y envía respuesta automáticamente)
        if procesar_update_telegram(update):
            return jsonify({"success": True}), 200
        else:
            return jsonify({"success": False, "message": "No se pudo procesar"}), 400
...
# ==============================================
# 🔧 ENDPOINT: CONFIGURAR WEBHOOK DE TELEGRAM
# ==============================================
...
# ==============================================
# 📊 ENDPOINT: ESTADÍSTICAS DE NOTIFICACIONES (temporalmente deshabilitado)
# ==============================================
...
# El __init__.py del módulo ya carga las rutas automáticamente
...
def actualizar_sesion_activa():
    ...
    if 'usuario_id' in session and 'usuario' in session:
        try:
            ...
            session_id = session.get('_id', 'unknown')  # ✅ CORREGIDO: request.session no existe en Flask
            ...
            if sesion:
                ...
                sesion.fecha_ultima_actividad = datetime.now()
...
def limpiar_sesiones_inactivas(response):
    ...
    try:
        ...
        # Marcar como inactivas las sesiones sin actividad > 20 minutos
        tiempo_limite = datetime.now() - timedelta(minutes=20)
...
def heartbeat_sesion():
    """Endpoint ligero llamado cada 2 min desde el frontend para mantener la sesión viva"""
    if 'usuario_id' not in session:
    ...
    try:
        ...
        ip_address = request.environ.get('HTTP_X_FORWARDED_FOR', request.environ.get('REMOTE_ADDR', 'unknown'))
        sesion = SesionActiva.query.filter_by(
            usuario_id=usuario_id, ip_address=ip_address, conectado=True
        ).first()
        if sesion:
            sesion.fecha_ultima_actividad = datetime.now()
            db.session.commit()
...
if __name__ == "__main__":
    # Registrar credenciales SMB automáticamente al iniciar
    try:
        import subprocess
        _smb_server = os.getenv('CARPETA_SERVIDOR', '')
        _smb_user = os.getenv('CARPETA_USUARIO', '')
        _smb_pass = os.getenv('CARPETA_PASSWORD', '')
        if _smb_server and _smb_user and _smb_pass:
            subprocess.run(
                ['cmdkey', f'/add:{_smb_server}', f'/user:{_smb_user}', f'/pass:{_smb_pass}'],
                capture_output=True
            )
            print(f"[OK] Credenciales de red registradas para {_smb_server}")
    except Exception as e:
        print(f"[ADVERTENCIA] No se pudieron registrar credenciales SMB: {e}")

    with app.app_context():
        try:
            db.create_all()
        except Exception as e:
            # Si hay error al crear tablas (ej: índices ya existen), continuar de todas formas
            log_security(f"ADVERTENCIA al crear tablas: {str(e)}")
...
### para iniciar el servidor desde la consola de visual estudio ###
#  PS D:\0.1. Backup Equipo Contablilidad\Gestor Documental\GESTOR_DOCUMENTAL_TRANSPORTABLE_20251113_204059> .\.venv\Scripts\python.exe app.py 
