"""
CARGA COMPLETA Y OPTIMIZADA DE TODAS LAS TABLAS
DIAN (66K) + ERP Financiero (3K) + Acuses (51K)
Con lógica de detección CORREGIDA
"""
import polars as pl
import pandas as pd
import os
...
print("\n" + "="*80)
print("CARGA COMPLETA Y OPTIMIZADA DE TODAS LAS TABLAS")
print("="*80)
...
with engine.connect() as conn:
    result = conn.execute(text("""
        ...
        AND table_name IN ('dian', 'erp_financiero', 'acuses_recibidos')
    """))
    tablas_existentes = [row[0] for row in result]
    print(f"  Tablas disponibles: {tablas_existentes}")

# ==============================================================================
# TABLA 1: DIAN (66,276 registros)
# ==============================================================================

if 'dian' in tablas_existentes:
    print("\n" + "-"*80)
    print("CARGANDO TABLA DIAN (66K REGISTROS)")
    print("-"*80)

    archivo_dian = r'uploads\dian\Dian_23022026.xlsx'

    print(f"Leyendo Excel DIAN (13.96 MB)... esto puede tomar 30-60 segundos...")
    inicio = datetime.now()
    ...
    print(f"  Columnas detectadas: nit={nit_col is not None}, razon={razon_col is not None}, numero={numero_col is not None}")

    # Procesar
    print(f"\nProcesando {len(df_dian):,} registros de DIAN...")
    ...
    # Insertar
    print(f"\nInsertando {len(registros_dian):,} registros en PostgreSQL...")
    ...
    cursor.execute("""
        ...
        SELECT nit_adquiriente, razon_social, numero, prefijo, folio, fecha_factura, valor, clave_dian
...
# ==============================================================================
# TABLA 2: ERP FINANCIERO (3,180 registros)
# ==============================================================================

if 'erp_financiero' in tablas_existentes:
    print("\n" + "-"*80)
    print("CARGANDO TABLA ERP FINANCIERO (3K REGISTROS)")
    print("-"*80)

    archivo_erp_fn = r'uploads\erp_fn\erp_financiero_23022026.xlsx'

    print("Leyendo Excel ERP Financiero...")
    ...
    print(f"  Columnas detectadas: proveedor={proveedor_col is not None}, clase={clase_col is not None}, fecha={fecha_col is not None}, valor={valor_col is not None}")

    # Procesar
    print(f"\nProcesando {len(df_fn):,} registros...")

    registros_fn = []
    for idx, (_, row) in enumerate(df_fn.iterrows()):
