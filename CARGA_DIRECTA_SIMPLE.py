...
def procesar_archivo_dian(archivo_path):
    ...
    for idx, row in df_pd.iterrows():
        ...
        # Buscar columna fecha con variantes (con/sin acento): "fecha emisión", "fecha emision", "fecha_emision"
        ...
        folio_raw = str(row.get('folio', row.get('numero', '')))
        folio = extraer_folio(folio_raw)
        
        # Buscar columna valor con variantes: "total", "valor"
        valor_raw = row.get('total', row.get('valor', 0))
        valor = float(valor_raw) if valor_raw and str(valor_raw).strip() != '' else 0.0
        
        tipo_doc = str(row.get('tipo de documento', row.get('tipo documento', row.get('tipo_documento', 'Factura Electrónica'))))
        cufe = str(row.get('cufe/cude', row.get('cufe', '')))
        # Buscar columna con diferentes variantes: "forma de pago", "forma pago", "forma_pago"
